...
#ifndef __gxos_MEM_TRACE_H__
#define __gxos_MEM_TRACE_H__
...
enum {
    gxos_MEM_TRACE_MALLOC,
    gxos_MEM_TRACE_REALLOC,
    gxos_MEM_TRACE_CALLOC,
    gxos_MEM_TRACE_FREE
};

/* Prefix for the output of the default tracer */
#define gxos_MEM_DEFAULT_TRACER_PREFIX  "#"
 ...
 *
 * @param op the ID of the operation (gxos_MEM_TRACE_MALLOC, gxos_MEM_TRACE_REALLOC,
 *           gxos_MEM_TRACE_CALLOC or gxos_MEM_TRACE_FREE).
 ...
 *               unreliable.
 *
 * The rest of the parameters passed 'gxos_mem_trace_cb_t' are the same as the memory operations
 * that triggered its call (see 'man malloc' for details):
 *
 * - for malloc: cb(gxos_MEM_TRACE_MALLOC, res, caller, size).
 * - for realloc: cb(gxos_MEM_TRACE_REALLOC, res, caller, ptr, size).
 * - for calloc: cb(gxos_MEM_TRACE_CALLOC, res, caller, nmemb, size).
 * - for free: cb(gxos_MEM_TRACE_FREE, NULL, caller, ptr).
 */
typedef void (*gxos_mem_trace_cb_t)(uint8_t op, void *res, void* caller, ...);
 ...
 */
void gxos_mem_trace_set_callback(gxos_mem_trace_cb_t cb);
 ...
 */
void *gxos_mem_trace_malloc(void *res, size_t size, void *caller);
 ...
 * @return 'res' (the first argument).
 */
void *gxos_mem_trace_realloc(void *res, void *ptr, size_t size, void *caller);
 ...
 * @return 'res' (the first argument).
 */
void *gxos_mem_trace_calloc(void *res, size_t num, size_t size, void *caller);
 ...
 * @param caller the caller of the memory operation.
 */
void gxos_mem_trace_free(void *ptr, void *caller);

/**
 * Default memory trace callback. DO NOT CALL DIRECTLY. It is meant to be used
 * as the second argument of 'gxos_mem_trace_setup'.
 *
 ...
 */
void gxos_mem_trace_default_callback(uint8_t op, void *res, void *caller, ...);

#ifdef __cplusplus
}
#endif

#endif// #ifndef __gxos_MEM_TRACE_H__
