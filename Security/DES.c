...
void CBCe(des_ctx *dc,unsigned char* pucData,unsigned char* pIV, short sBlocks, unsigned char *pucKey1, unsigned char *pucKey2, unsigned char *pucKey3){
	int i; 
	int j;
  for (i=0; i<sBlocks; i++){
    for(j=0; j < 8; j++){
		pucData[i*8+j] = pucData[i*8+j] ^ pIV[j];// we have input block to Enc block	
		}
		TripleDES_ENC(dc,pucData+(i*8),1,pucKey1,pucKey2,pucKey3);
		for(j=0; j < 8; j++){
		pIV[j] = pucData[i*8+j];// updating pIV	
		}
	}
}
void CBCd(des_ctx *dc,unsigned char* pucData,unsigned char* pIV, short sBlocks, unsigned char *pucKey1, unsigned char *pucKey2, unsigned char *pucKey3){
	int i; 
	int j;
	unsigned char buffer[8];
	for (i=0; i<sBlocks; i++){
		for(j=0; j < 8; j++){
			buffer[j] = pucData[i*8+j];// updating pIV	
		}
		TripleDES_DEC(dc,pucData+(i*8),1,pucKey1,pucKey2,pucKey3);
		for(j=0; j < 8; j++){
			pucData[i*8+j] = pucData[i*8+j] ^ pIV[j];// we have input block to Enc block	
		}
		for(j=0; j < 8; j++){
			pIV[j] = buffer[j];// updating pIV	
		}
	}
}
